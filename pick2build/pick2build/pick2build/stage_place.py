            ...
            if triggered:
                ...
                if response is not None and response.success:
                    ...
                    if obj:
                        ...
                        # 💡 [핵심 함수] 비전 탐색을 위한 안전한 통신 내부 함수
                        def get_vision_target(target_name):
                            ...
                            if not v_client.wait_for_service(timeout_sec=3.0):
                                self.get_logger().error("비전 노드를 찾을 수 없습니다.")
                                temp_vision.destroy_node()
                                return None
                            
                            v_req = SrvDepthPosition.Request()
                            v_req.target = target_name # 찾고자 하는 타겟(hand 또는 obj) 전달
                            ...
                            if v_res is not None:
                                ...
                                if sum(cam_coords) != 0:
                                    # 예전 코드에 맞춰 패키지 경로를 수정 (필요시 pick2build로 변경)
                        ...
                        if tgt.lower() == 'hand':
                            ...
                            hand_pos = get_vision_target('hand')
                            if not hand_pos:
                        ...
                        tool_pos = get_vision_target(obj)
            ...
            try:
                ...
                if self.brick_mesh == 0 :
                    ...
                    def get_vision_target(target_name):
                        temp_vision = rclpy.create_node(f'temp_vision_{int(time.time())}')
                        v_client = temp_vision.create_client(SrvDepthPosition, 'get_3d_position')
                        if not v_client.wait_for_service(timeout_sec=10.0):
                            self.get_logger().error("비전 노드를 찾을 수 없습니다.")
                            temp_vision.destroy_node()
                            return None
